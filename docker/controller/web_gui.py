...
def _arp_mac(ip: str) -> str:
    """Read the current ARP-cached MAC for an IP from /proc/net/arp."""
    try:
...
shared_state = {
    "registers": {u: {} for u in range(1, 9)},
    "register_names": {},
    "poll_log": [],
    "cmd_log": [],
    # Controlled setpoints — keyed (unit, addr). Updated by lever/rudder writes
    # so the controller's write cycle uses the operator's latest commanded value.
    "setpoints": {},
    "fd_info": [],   # list of {ip, port, units, mac, connected}
}
...
def api_state():
    with state_lock:
        return jsonify({
            ...
            "fd_info": shared_state["fd_info"],
        })


@app.route("/api/fd_info")
def api_fd_info():
    """Live field device info including current ARP-cached MACs."""
    devices = [
        {"ip": FIELD_DEVICE_HOST,   "port": FIELD_DEVICE_PORT,   "units": "1–4", "label": "FD1"},
        {"ip": FIELD_DEVICE_2_HOST, "port": FIELD_DEVICE_2_PORT, "units": "5–8", "label": "FD2"},
    ]
    for d in devices:
        d["mac"] = _arp_mac(d["ip"])
...
def api_command():
    ...
    try:
        ...
        if fc == "read_holding":
            ...
            if not rr.isError():
                with state_lock:
                    shared_state["setpoints"][(unit, addr)] = val
            ...
            if not rr.isError():
                with state_lock:
                    for i, v in enumerate(vals):
                        shared_state["setpoints"][(unit, addr + i)] = v
...
def on_connect():
    with state_lock:
        ...
        socketio.emit("fd_info", {"devices": shared_state["fd_info"]})
